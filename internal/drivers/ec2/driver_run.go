...
func (d *Driver) Run(ctx context.Context, name name.Reference) (*drivers.RunResult, error) {
	...
	// Pull the image.
	pullResult, err := client.ImagePull(ctx, name.String(), image.PullOptions{
		All:          true,
	...
	for {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("failed log read: %w", err)
		}
		if n == 0 {
			continue
		}
		...
		if err != nil {
			return nil, fmt.Errorf("failed to write log output to buffer: %w", err)
		}
		fmt.Print(string(buf[:n]))
	}
	log.Info("log read complete")
